...
import { cleanAndMerge } from './chunk-OEGWB26X.js';
...
import { __name, log } from './chunk-XXDMU5XJ.js';
...
var populateDb = __name((ast, db) => {
  ...
  ast.components.forEach((component) => {
    ...
    const sourceStrategy =
      (_a2 = component.decorator) == null ? void 0 : _a2.strategy;
    db.addNode(
      component.name,
      component.name,
      coords.x,
  ...
  ast.links.forEach((link) => {
    ...
    db.addLink(
      db.resolveNodeId(link.from),
      db.resolveNodeId(link.to),
      isDashed,
      label,
      flow,
    );
...
var getTheme = __name(() => {
  ...
  return {
    backgroundColor:
      ...
      '#fff',
    axisColor:
      ((_b = themeVariables.wardley) == null ? void 0 : _b.axisColor) ?? '#000',
    axisTextColor:
      ((_c = themeVariables.wardley) == null ? void 0 : _c.axisTextColor) ??
      themeVariables.primaryTextColor ??
      '#222',
    gridColor:
      ((_d = themeVariables.wardley) == null ? void 0 : _d.gridColor) ??
      'rgba(100, 100, 100, 0.2)',
    componentFill:
      ((_e = themeVariables.wardley) == null ? void 0 : _e.componentFill) ??
      '#fff',
    componentStroke:
      ((_f = themeVariables.wardley) == null ? void 0 : _f.componentStroke) ??
      '#000',
    componentLabelColor:
      ((_g = themeVariables.wardley) == null
        ? void 0
        : _g.componentLabelColor) ??
      themeVariables.primaryTextColor ??
      '#222',
    linkStroke:
      ((_h = themeVariables.wardley) == null ? void 0 : _h.linkStroke) ??
      '#000',
    evolutionStroke:
      ((_i = themeVariables.wardley) == null ? void 0 : _i.evolutionStroke) ??
      '#dc3545',
    annotationStroke:
      ((_j = themeVariables.wardley) == null ? void 0 : _j.annotationStroke) ??
      '#000',
    annotationTextColor:
      ((_k = themeVariables.wardley) == null
        ? void 0
        : _k.annotationTextColor) ??
      themeVariables.primaryTextColor ??
      '#222',
    annotationFill:
...
var getConfigValues = __name(() => {
  ...
  return {
    ...
    nodeRadius:
      (wardleyConfig == null ? void 0 : wardleyConfig.nodeRadius) ?? 6,
    nodeLabelOffset:
      (wardleyConfig == null ? void 0 : wardleyConfig.nodeLabelOffset) ?? 8,
    axisFontSize:
      (wardleyConfig == null ? void 0 : wardleyConfig.axisFontSize) ?? 12,
    labelFontSize:
      (wardleyConfig == null ? void 0 : wardleyConfig.labelFontSize) ?? 10,
    showGrid:
      (wardleyConfig == null ? void 0 : wardleyConfig.showGrid) ?? false,
    useMaxWidth:
      (wardleyConfig == null ? void 0 : wardleyConfig.useMaxWidth) ?? true,
  };
}, 'getConfigValues');
var draw = __name((text, id, _version, diagObj) => {
  ...
  const width =
    ((_a2 = data.size) == null ? void 0 : _a2.width) ?? configValues.width;
  const height =
    ((_b = data.size) == null ? void 0 : _b.height) ?? configValues.height;
  ...
  const validLinks = data.links.filter((link) => {
    ...
    if (
      pipelineComponents == null ? void 0 : pipelineComponents.has(link.source)
    ) {
      return false;
    }
    return true;
  ...
  linksGroup
    ...
    .attr(
      'class',
      (link) => `wardley-link${link.dashed ? ' wardley-link--dashed' : ''}`,
    )
    .attr('x1', (link) => {
  ...
  const trendsWithPositions = data.trends
    .map((trend) => {
      ...
      const adjustedX2 =
        distance > shortenBy ? targetX - (dx / distance) * shortenBy : targetX;
      const adjustedY2 =
        distance > shortenBy ? targetY - (dy / distance) * shortenBy : targetY;
      return {
        origin,
        targetX,
        targetY,
  ...
  const marketNodes = nodeEnter.filter(
    (node) => node.sourceStrategy === 'market',
  );
  marketNodes
    .append('circle')
  ...
  marketNodes
    ...
    .attr(
      'x2',
      (node) =>
        positions.get(node.id).x - triangleRadius * Math.cos(Math.PI / 6),
    )
    .attr(
      'y2',
      (node) =>
        positions.get(node.id).y + triangleRadius * Math.sin(Math.PI / 6),
    )
    .attr('stroke', theme.componentStroke)
    .attr('stroke-width', 1);
  marketNodes
  ...
  nodeEnter
    ...
    .attr('y1', (node) => {
      const pos = positions.get(node.id);
      const lineHeight = node.isPipelineParent
        ? squareSize
        : configValues.nodeRadius * 2;
  ...
  nodeEnter
    ...
    .attr('y', (node) => {
      const pos = positions.get(node.id);
      if (node.className === 'anchor') {
        return node.labelOffsetY !== void 0
    ...
    .attr('font-weight', (node) =>
      node.className === 'anchor' ? 'bold' : 'normal',
    )
    .attr('text-anchor', (node) =>
      node.className === 'anchor' ? 'middle' : 'start',
    )
    .attr('dominant-baseline', (node) =>
      node.className === 'anchor' ? 'middle' : 'auto',
    )
  ...
  if (data.annotations.length > 0) {
    ...
    if (data.annotationsBox) {
      ...
      const lineHeight = 16;
      const fontSize = 11;
      const textBoxGroup = annotationsGroup
        .append('g')
      ...
      if (textElements.length > 0) {
        ...
        const boxHeight =
          sortedAnnotations.length * lineHeight + padding * 2 + maxHeight / 2;
...
export { diagram };
//# sourceMappingURL=wardleyDiagram-YWT4CUSO-YGPDG2DQ.js.map
