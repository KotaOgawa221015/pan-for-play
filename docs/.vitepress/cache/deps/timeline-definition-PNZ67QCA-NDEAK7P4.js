...
import { parseFontSize } from './chunk-OEGWB26X.js';
...
var addTask = __name((period, length, event) => {
  const rawTask = {
    id: currentTaskId++,
...
var addEvent = __name((event) => {
  const currentTask = rawTasks.find((task) => task.id === currentTaskId - 1);
  currentTask.events.push(event);
}, 'addEvent');
var addTaskOrg = __name((descr) => {
  const newTask = {
    section: currentSection,
    type: currentSection,
    description: descr,
...
var compileTasks = __name(() => {
  const compileTask = __name((pos) => rawTasks[pos].processed, 'compileTask');
...
var nodeCount = 0;
var drawRect = __name((elem, rectData) => {
...
var drawFace = __name((element, faceData) => {
  const radius = 15;
...
var drawCircle = __name((element, circleData) => {
...
var drawSection = __name((elem, section, conf) => {
...
var taskCount = -1;
var drawTask = __name((elem, task, conf, diagramId) => {
...
var drawBackgroundRect = __name((elem, bounds) => {
...
var _drawTextCandidateFunc = (() => {
  ...
  return (conf) =>
    conf.textPlacement === 'fo'
      ? byFo
      : conf.textPlacement === 'old'
        ? byText
        : byTspan;
})();
var initGraphics = __name((graphics, id) => {
  nodeCount = 0;
  taskCount = -1;
  graphics
    .append('defs')
...
__name(wrap, 'wrap');
var drawNode = __name(
  (elem, node, fullSection, conf, diagramId, isEvent = false) => {
    var _a, _b, _c;
    const { theme, look } = conf;
    const isReduxTheme = theme == null ? void 0 : theme.includes('redux');
    const maxSections =
      ((_a = conf == null ? void 0 : conf.themeVariables) == null
        ? void 0
        : _a.THEME_COLOR_LIMIT) ?? 12;
    ...
    if (look === 'neo') {
      ...
      if (isReduxTheme) {
        ...
        const rootSvgNode =
          ((_c = elem.node()) == null ? void 0 : _c.ownerSVGElement) ??
          elem.node();
        ...
        if (rootSvg.select(`#${dropShadowId}`).empty()) {
          ...
          const defsEl = existingDefs.empty()
            ? rootSvg.append('defs')
            : existingDefs;
          defsEl
            .append('filter')
...
var getVirtualNodeHeight = __name((elem, node, conf) => {
  var _a;
...
var defaultBkg = __name((elem, node, section, diagramId, config) => {
  const { theme } = config;
...
var draw = __name((text, id, version, diagObj) => {
  ...
  if (title) {
    svg
      ...
      .attr(
        'x',
        look === 'neo' ? box.x * 2 + LEFT_MARGIN : box.width / 2 - LEFT_MARGIN,
      )
      .attr('font-size', '4ex')
...
var drawEvents = __name(
  (diagram2, events, sectionColor, masterX, masterY, conf, diagramId) => {
...
var draw2 = __name((text, id, version, diagObj) => {
  var _a, _b, _c, _d, _e;
  const conf = getConfig2();
  const LEFT_MARGIN =
    ((_a = conf.timeline) == null ? void 0 : _a.leftMargin) ?? 50;
  ...
  for (const [i, task] of tasks2.entries()) {
    ...
    for (const event of task.events) {
      ...
      maxEventStackHeightTemp += svgDraw_default.getVirtualNodeHeight(
        svg,
        eventNode,
        conf,
      );
    ...
    maxEventStackHeight =
      Math.max(maxEventStackHeight, maxEventStackHeightTemp) +
      EVENT_VERTICAL_GAP;
  }
  log.debug('maxSectionHeight before draw', maxSectionHeight);
  log.debug('maxTaskHeight before draw', maxTaskHeight);
...
var drawEvents2 = __name(
  (diagram2, events, sectionColor, axisX, eventsX, startY, conf) => {
    let currentY = startY;
    for (const event of events) {
      const eventNode = {
        descr: event,
...
var genReduxSections = __name((options) => {
  ...
  const rawSvgId =
    ((_a = options.svgId) == null ? void 0 : _a.replace(/^#/, '')) ?? '';
  ...
  for (let i = 0; i < options.THEME_COLOR_LIMIT; i++) {
    ...
    const stroke = isColorTheme
      ? options.borderColorArray[i]
      : options.nodeBorder;
    sections2 += `
    .section-${i - 1} rect,
    .section-${i - 1} path,
...
var rendererSelector = {
  ...
  draw: __name((text, id, version, diagObj) => {
    ...
    const direction2 =
      ((_b =
        (_a = diagObj == null ? void 0 : diagObj.db) == null
          ? void 0
          : _a.getDirection) == null
        ? void 0
        : _b.call(_a)) ?? 'LR';
    if (direction2 === 'TD') {
...
export { diagram };
//# sourceMappingURL=timeline-definition-PNZ67QCA-NDEAK7P4.js.map
