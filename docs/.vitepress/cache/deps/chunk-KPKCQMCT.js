import {
  ...
  styles2String,
  userNodeOverrides,
} from './chunk-DAFQSPID.js';
import {
  configureLabelImages,
  getSubGraphTitleMargins,
} from './chunk-XISVJL4A.js';
import { at } from './chunk-B7KO5YQV.js';
import { createText, getIconSVG } from './chunk-5PGEKNL7.js';
import {
  calculateTextWidth,
  decodeEntities,
  handleUndefinedAttr,
...
function mergePaths(roughElement) {
  ...
  const mergedPath = document.createElementNS(
...
var rect = __name(async (parent, node) => {
  ...
  node.intersect = (point) => intersect_rect_default(node, point);
...
function intersectPolygon(node, polyPoints, point) {
  ...
  for (let i = 0; i < polyPoints.length; i++) {
    ...
    const intersect = intersect_line_default(
      node,
      point,
...
function calculateArcSagitta(chord, radiusX, radiusY) {
  const [semiMajorAxis, semiMinorAxis] = [radiusX, radiusY].sort(
    (a, b) => b - a,
  );
...
async function bowTieRect(parent, node) {
  ...
  const calcTotalHeight = __name(
    (labelHeight) => labelHeight + labelPaddingY,
    'calcTotalHeight',
  );
...
async function card(parent, node) {
  ...
  node.intersect = (point) => intersect_default.polygon(node, points, point);
  return shapeSvg;
}
__name(card, 'card');
...
async function circle(parent, node, options) {
  ...
  updateNodeBounds(node, circleElem);
  node.calcIntersect = (bounds, point) => {
...
async function curlyBraceRight(parent, node) {
  ...
  const w =
    bbox.width + (node.look === 'neo' ? labelPaddingX * 2 : labelPaddingX);
  const h =
...
async function drawRect(parent, node, options) {
  ...
  node.calcIntersect = (bounds, point) => intersect_default.rect(bounds, point);
  node.intersect = (point) => intersect_default.rect(node, point);
  return shapeSvg;
}
__name(drawRect, 'drawRect');
async function datastore(parent, node) {
  const { cssClasses, labelPaddingX, labelPaddingY, padding, width, height } =
    node;
  const rectOptions = {
    rx: 0,
    ry: 0,
  ...
  if (node.look === 'handDrawn') {
    ...
    const borderSelection = rect2.select(
      '.basic.label-container > path:nth-child(2)',
    );
...
async function hexagon(parent, node) {
  ...
  if (node.width || node.height) {
    ...
    node.width =
      ((node == null ? void 0 : node.width) ?? 0) - 2 * m2 - labelPaddingY;
...
async function lean_left(parent, node) {
  ...
  const h =
    ((node == null ? void 0 : node.height) ?? bbox.height) + labelPaddingY;
  const w =
    ((node == null ? void 0 : node.width) ?? bbox.width) + labelPaddingX;
  const points = [
    { x: 0, y: 0 },
    { x: w + (3 * h) / 6, y: 0 },
    { x: w, y: -h },
...
async function linedWaveEdgedRect(parent, node) {
  ...
  if (node.width || node.height) {
    ...
    node.height =
      ((node == null ? void 0 : node.height) ?? 0) - labelPaddingY * 2;
    if (node.height < 10) {
      node.height = 10;
    }
  }
...
async function multiWaveEdgedRectangle(parent, node) {
  ...
  if (node.width || node.height) {
    ...
    node.width =
      ((node == null ? void 0 : node.width) ?? 0) - labelPaddingX * 2;
    node.height =
      ((node == null ? void 0 : node.height) ?? 0) - labelPaddingY * 3;
  }
  const w =
    Math.max(bbox.width, (node == null ? void 0 : node.width) ?? 0) +
    labelPaddingX * 2;
  const h =
    Math.max(bbox.height, (node == null ? void 0 : node.height) ?? 0) +
    labelPaddingY * 3;
  const waveAmplitude = node.look === 'neo' ? h / 4 : h / 8;
  const finalH =
    h + (adjustFinalHeight ? waveAmplitude / 2 : -waveAmplitude / 2);
  const x = -w / 2;
  const y = -finalH / 2;
...
async function shadedProcess(parent, node) {
  ...
  const totalHeight =
    ((node == null ? void 0 : node.height) ?? bbox.height) + paddingY * 2;
...
function stateEnd(parent, node, { config: { themeVariables } }) {
  ...
  node.intersect = (point) =>
    intersect_default.circle(node, (node.width ?? 0) / 2, point);
  return shapeSvg;
}
...
function stateStart(parent, node, { config: { themeVariables } }) {
  ...
  updateNodeBounds(node, circle2);
  node.intersect = (point) =>
    intersect_default.circle(node, (node.width ?? 7) / 2, point);
  return shapeSvg;
}
...
async function triangle(parent, node) {
  ...
  const useHtmlLabels = evaluate(
    (_a = getConfig2().flowchart) == null ? void 0 : _a.htmlLabels,
  );
  const w =
    ((node == null ? void 0 : node.width)
      ? node == null
...
async function waveRectangle(parent, node) {
  ...
  if (node.width || node.height) {
    ...
    node.height = Math.ceil(
      node.height - labelPaddingY - waveAmplitude2 * (20 / 9),
    );
...
async function erBox(parent, node) {
  ...
  if (entityNode.attributes.length === 0 && node.label) {
    ...
    if (theme != null && COLOR_THEMES.has(theme)) {
      ...
      shapeSvg2.attr(
        'data-color-id',
        `color-${colorIndex % borderColorArray.length}`,
      );
    }
...
async function classBox(parent, node) {
  ...
  shapeSvg.selectAll('.text').each((_, i, nodes) => {
    ...
    if (text2.attr('class').includes('methods-group')) {
      const membersGroupHeightForMethods = Math.max(
        membersGroupHeight,
        GAP / 2,
      );
      if (nodeHeightGreater) {
    ...
    if (
      ...
      newTranslateX =
        -((_b2 = text2.node()) == null ? void 0 : _b2.getBBox().width) / 2 || 0;
      shapeSvg.selectAll('text').each((_2, i2, nodes2) => {
  ...
  if (
    ...
    line
      .attr(
        'class',
        `divider${node.look === 'neo' && !useGradient ? ' neo-line' : ''}`,
      )
      .attr('style', styles);
  }
  if (
    ...
    const roughLine = rc.line(
      rectBBox.x,
      nodeHeightGreater
        ? Math.max(methodsAreaPlacement, secondLineY)
        : secondLineY,
      rectBBox.x + rectBBox.width,
      (nodeHeightGreater
        ? Math.max(methodsAreaPlacement, secondLineY)
...
async function kanbanItem(parent, kanbanNode, { config }) {
  ...
  kanbanNode.intersect = (point) => intersect_default.rect(kanbanNode, point);
  return shapeSvg;
}
...
//# sourceMappingURL=chunk-KPKCQMCT.js.map
