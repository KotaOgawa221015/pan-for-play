import { ImperativeState } from './chunk-6RROGHUU.js';
import {
  drawBackgroundRect,
  drawEmbeddedImage,
  drawImage,
  ...
  getNoteRect,
  getTextObj,
} from './chunk-GK34SFUZ.js';
import { JSON_SCHEMA, load } from './chunk-3GNSFRHJ.js';
import {
  ZERO_WIDTH_SPACE,
  parseFontSize,
  utils_default,
} from './chunk-OEGWB26X.js';
      ...
      conditions: {
        ...
        INITIAL: {
          rules: [
            ...
            48, 50, 52, 55, 56, 57, 58, 59, 60, 61, 62, 63, 64, 65, 66, 67, 68,
...
var drawRect2 = __name((elem, rectData) => {
...
var drawPopup = __name((elem, actor, minMenuWidth, textAttrs, forceMenus) => {
  if (
    actor.links === void 0 ||
    actor.links === null ||
...
var drawKatex = __name(async (elem, textData, msgModel = null) => {
...
var drawText = __name((elem, textData) => {
...
var drawLabel = __name((elem, txtObject) => {
...
var drawActorTypeParticipant = __name(
  (elem, actor, conf2, isFooter, actorIndexMap) => {
    var _a2, _b;
    ...
    if (COLOR_THEMES.has(theme)) {
      rectElem.style(
        'stroke',
        borderColorArray[actorCount % borderColorArray.length],
      );
      rectElem.style(
        'fill',
        bkgColorArray[actorCount % borderColorArray.length],
      );
    }
    if (look === 'neo') {
...
var drawActorTypeCollections = __name(
  (elem, actor, conf2, isFooter, actorIndexMap) => {
    var _a2, _b;
    const actorY = isFooter ? actor.stopy : actor.starty;
    const center = actor.x + actor.width / 2;
...
var drawActorTypeControl = __name(
  (elem, actor, conf2, isFooter, diagramId, actorIndexMap) => {
    var _a2;
    const actorY = isFooter ? actor.stopy : actor.starty;
    const center = actor.x + actor.width / 2;
    const centerY = actorY + 75;
    const { look, theme, themeVariables } = conf2;
    const { bkgColorArray, borderColorArray, actorBorder, actorBkg } =
      themeVariables;
...
var drawActorTypeEntity = __name(
  (elem, actor, conf2, isFooter, actorIndexMap) => {
    var _a2;
    const actorY = isFooter ? actor.stopy : actor.starty;
    const center = actor.x + actor.width / 2;
...
var drawActorTypeDatabase = __name(
  (elem, actor, conf2, isFooter, actorIndexMap) => {
    var _a2;
    const actorY = isFooter ? actor.stopy : actor.starty;
    const center = actor.x + actor.width / 2;
...
var drawActorTypeBoundary = __name(
  (elem, actor, conf2, isFooter, actorIndexMap) => {
    const actorY = isFooter ? actor.stopy : actor.starty;
    const center = actor.x + actor.width / 2;
...
var drawActor = __name(
  async (elem, actor, conf2, isFooter, diagramId, diagObj, actorIndexMap) => {
    const resolvedActorIndexMap =
      actorIndexMap ??
      new Map(
        [...diagObj.db.getActors().values()].map((participant, index) => [
          participant.name,
          index,
        ]),
      );
    switch (actor.type) {
      case 'actor':
        return await drawActorTypeActor(
          elem,
          actor,
          conf2,
          isFooter,
...
var drawBox = __name((elem, box, conf2) => {
...
var anchorElement = __name((elem) => elem.append('g'), 'anchorElement');
var drawActivation = __name(
  (
    _elem,
    bounds2,
    verticalPos,
    ...
    if (COLOR_THEMES.has(theme)) {
      ...
      rectElem.style(
        'fill',
        bkgColorArray[actorCount % borderColorArray.length] ?? mainBkg,
      );
    }
  },
  'drawActivation',
);
var drawLoop = __name(async (elem, loopModel, labelText, conf2, msg) => {
  const {
    boxMargin,
    boxTextMargin,
    labelBoxHeight,
  ...
  txt.fontWeight = fontWeight;
  txt.wrap = true;
  const textElem = hasKatex(txt.text)
...
var drawBackgroundRect2 = __name((elem, bounds2) => {
  drawBackgroundRect(elem, bounds2);
}, 'drawBackgroundRect');
var insertDatabaseIcon = __name((elem, id) => {
  elem
    .append('defs')
    .append('symbol')
    .attr('id', id + '-database')
...
var insertComputerIcon = __name((elem, id) => {
  elem
    .append('defs')
    .append('symbol')
    .attr('id', id + '-computer')
...
var insertClockIcon = __name((elem, id) => {
  elem
    .append('defs')
    .append('symbol')
    .attr('id', id + '-clock')
...
var insertArrowHead = __name((elem, id) => {
  elem
    .append('defs')
    .append('marker')
...
var insertArrowFilledHead = __name((elem, id) => {
  elem
    .append('defs')
    .append('marker')
...
var insertSequenceNumber = __name((elem, id) => {
  elem
    .append('defs')
    .append('marker')
...
var insertArrowCrossHead = __name((elem, id) => {
...
var insertDropShadow = __name((elem, conf2) => {
  const { theme } = conf2;
  elem
    .append('defs')
    .append('filter')
...
var _drawMenuItemTextCandidateFunc = (() => {
  function byText(content, g, x, y, width, height, textAttrs) {
    const text = g
      .append('text')
      .attr('x', x)
  ...
  __name(_setTextAttrs, '_setTextAttrs');
  return (conf2) =>
    conf2.textPlacement === 'fo'
      ? byFo
      : conf2.textPlacement === 'old'
        ? byText
        : byTspan;
})();
var insertSolidTopArrowHead = __name((elem, id) => {
  elem
    .append('defs')
    .append('marker')
...
var insertSolidBottomArrowHead = __name((elem, id) => {
  elem
    .append('defs')
    .append('marker')
...
var insertStickTopArrowHead = __name((elem, id) => {
  elem
    .append('defs')
    .append('marker')
...
var insertStickBottomArrowHead = __name((elem, id) => {
  elem
    .append('defs')
    .append('marker')
...
var bounds = {
  ...
  updateVal: __name((obj, key, val, fun) => {
    if (obj[key] === void 0) {
      obj[key] = val;
    } else {
...
var drawNote = __name(async (elem, noteModel, id) => {
...
var drawCentralConnection = __name(
  (elem, msg, msgModel, diagObj, startx, stopx, lineStartY) => {
...
var drawMessage = __name(
  async (diagram2, msgModel, lineStartY, diagObj, msg, diagramId) => {
    const {
      startx,
      stopx,
      starty,
    ...
    if (
      ...
      line.attr(
        'marker-end',
        'url(' + url + '#' + diagramId + '-solidTopArrowHead)',
      );
    }
    if (
      ...
      line.attr(
        'marker-end',
        'url(' + url + '#' + diagramId + '-solidBottomArrowHead)',
      );
    }
    if (
      ...
      line.attr(
        'marker-end',
        'url(' + url + '#' + diagramId + '-stickTopArrowHead)',
      );
    }
    if (
      ...
      line.attr(
        'marker-end',
        'url(' + url + '#' + diagramId + '-stickBottomArrowHead)',
      );
    }
    if (
    ...
    if (sequenceVisible || conf.showSequenceNumbers) {
      ...
      diagram2
        ...
        .attr(
          'marker-start',
          'url(' + url + '#' + diagramId + '-sequencenumber)',
        );
      diagram2
        .append('text')
        .attr('x', autonumberX)
...
var addActorRenderingData = __name(
  (
    diagram2,
    actors,
    createdActors,
    ...
    for (const actorKey of actorKeys) {
      ...
      actor.width = common_default.getMax(
...
var drawActorsPopup = __name((diagram2, actors, actorKeys, doc) => {
  let maxHeight = 0;
  let maxWidth = 0;
  for (const actorKey of actorKeys) {
    const actor = actors.get(actorKey);
...
var actorActivations = __name(
  (actor) =>
    bounds.activations.filter((activation) => activation.actor === actor),
  'actorActivations',
);
var activationBounds = __name((actor, actors) => {
  const actorObj = actors.get(actor);
  const activations = actorActivations(actor);
  const left = activations.reduce(
    (acc, activation) => common_default.getMin(acc, activation.startx),
    actorObj.x + actorObj.width / 2 - 1,
  );
  const right = activations.reduce(
    (acc, activation) => common_default.getMax(acc, activation.stopx),
    actorObj.x + actorObj.width / 2 + 1,
  );
  return [left, right];
}, 'activationBounds');
...
var draw = __name(async (_text, id, _version, diagObj) => {
  ...
  const actorIndexMap = new Map(
    actorKeys.map((actorKey, index2) => {
      var _a2;
      return [
        ((_a2 = actors.get(actorKey)) == null ? void 0 : _a2.name) ?? actorKey,
        index2,
      ];
    }),
  );
  ...
  for (const msg of messages) {
    ...
    if (
      ...
      sequenceIndex =
        Math.round((sequenceIndex + sequenceIndexStep) * 100) / 100;
    }
    index++;
  }
...
var getRequiredPopupWidth = __name((actor) => {
...
var buildNoteModel = __name(async (msg, actors, diagObj) => {
  const fromActor = actors.get(msg.from);
  const toActor = actors.get(msg.to);
...
var hasCentralConnection = __name((msg, diagObj) => {
  const {
    CENTRAL_CONNECTION,
    CENTRAL_CONNECTION_REVERSE,
    CENTRAL_CONNECTION_DUAL,
...
var calculateCentralConnectionOffset = __name(
  (msg, diagObj, isArrowToRight) => {
    const {
      CENTRAL_CONNECTION_REVERSE,
      CENTRAL_CONNECTION_DUAL,
      BIDIRECTIONAL_SOLID,
...
var isReverseArrowType = __name((msg, diagObj) => {
  const {
...
var isBidirectionalArrowType = __name((msg, diagObj) => {
...
var buildMessageModel = __name((msg, actors, diagObj) => {
  const { look } = getConfig2();
  if (
    ![
      diagObj.db.LINETYPE.SOLID_OPEN,
...
var calculateLoopBounds = __name(
  async (messages, actors, _maxWidthPerActor, diagObj) => {
    const loops = {};
    const stack = [];
...
export { diagram };
//# sourceMappingURL=sequenceDiagram-3UESZ5HK-TBJKCWML.js.map
