...
import { cleanAndMerge, random, utils_default } from './chunk-OEGWB26X.js';
...
var setDirection = __name((dir2) => {
  state.records.direction = dir2;
}, 'setDirection');
var setOptions = __name((rawOptString) => {
...
var getOptions = __name(() => state.records.options, 'getOptions');
var commit = __name((commitDB) => {
...
var cherryPick = __name((cherryPickDB) => {
...
var getBranchesAsObjArray = __name(() => {
...
var getBranches = __name(() => state.records.branches, 'getBranches');
var getCommits = __name(() => state.records.commits, 'getCommits');
var getCommitsArray = __name(() => {
...
var getCurrentBranch = __name(
  () => state.records.currBranch,
  'getCurrentBranch',
);
var getDirection = __name(() => state.records.direction, 'getDirection');
var getHead = __name(() => state.records.head, 'getHead');
var db = {
  commitType,
...
var getCommitPosition = __name((commit2, pos, isParallelCommits) => {
  ...
  const branchY =
    (_a = branchPos.get(commit2.branch)) == null ? void 0 : _a.pos;
  const x =
    dir === 'TB' || dir === 'BT'
  ...
  const y =
    dir === 'TB' || dir === 'BT'
      ? posWithOffset
      : branchY +
        (useReduxGeometry ? REDUX_BRANCH_LABEL_PADDING_Y / 2 + 1 : -2);
...
var drawArrow = __name((svg, commitA, commitB, allCommits) => {
  ...
  svg
    ...
    .attr(
      'class',
      'arrow arrow' +
        calcColorIndex(colorClassNum, THEME_COLOR_LIMIT, useColorTheme),
    );
}, 'drawArrow');
  ...
  branches.forEach((branch2, index) => {
    ...
    bkg
      ...
      .attr(
        'style',
        look === 'neo'
          ? `filter:${useReduxGeometry ? `url(#${id}-drop-shadow)` : dropShadow}`
          : '',
      )
    ...
    if (dir === 'TB') {
      ...
      if (useReduxGeometry) {
        bkg.attr(
          'transform',
          `translate(${-labelPaddingX / 2 - 3}, ${-labelPaddingY - 10})`,
        );
        label.attr(
          'transform',
          'translate(' +
      ...
      bkg.attr(
        'transform',
        'translate(-19, ' + (spineY - 12 - labelPaddingY / 2) + ')',
      );
    }
  });
}, 'drawBranches');
var setBranchPosition = __name((name, pos, index, bbox, rotateCommitLabel) => {
...
var draw = __name((txt, id, ver, diagObj) => {
  ...
  const { look, theme, themeVariables } = getConfig2();
  const { useGradient, gradientStart, gradientStop, filterColor } =
    themeVariables;
  if (useGradient) {
...
export { diagram };
//# sourceMappingURL=gitGraphDiagram-PVQCEYII-23AL65NQ.js.map
