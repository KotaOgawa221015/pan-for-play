...
import { isEmResetFrame } from './chunk-7MUBAYHL.js';
import {
...
var setOptions = __name((_rawOptString) => {
  log.debug('options str', _rawOptString);
}, 'setOptions');
var getOptions = __name(() => ({}), 'getOptions');
var clear2 = __name(() => {
  reset();
  clear();
}, 'clear');
...
function getState() {
  ...
  ast.frames.forEach((frame, index) => {
    const textProps = calculateTextProps(
      frame,
      ast.dataEntities,
      diagramProps2,
    );
    ...
    if (hasSourceFrame(frame)) {
      ...
      sourceFrames = ast.frames.filter((currentFrame) => {
        return frame.sourceFrames.some(
          (sf) => sf.$refText === currentFrame.name,
        );
      });
...
function findSwimlaneByNamespace(swimlanes, namespace) {
  ...
  return Object.values(swimlanes).find(
    (swimlane) => swimlane.namespace === namespace,
  );
}
...
function calculateTextProps(frame, dataEntities, diagramProps2) {
  ...
  if (frame.dataReference) {
    const dataEntity = dataEntities.find((dataEntity2) => {
      ...
      return (
        dataEntity2.name ===
        ((_a = frame.dataReference) == null ? void 0 : _a.$refText)
      );
    });
    if (dataEntity) {
  ...
  const calculatedWidthFix = hasRenderedData
    ? dimensions.width / 3
    : dimensions.width;
  const props = {
    content,
...
function evolveFramePositioned(state, _event) {
  ...
  if (swimlaneProps.index in state.swimlanes) {
    ...
    swimlane = {
      ...
      r: 0,
      y:
        swimlaneProps.index * diagramProps.swimlaneMinHeight +
        diagramProps.swimlaneGap,
      height: diagramProps.swimlaneMinHeight,
      maxHeight: diagramProps.swimlaneMinHeight,
    };
  }
  const lastBox =
    state.boxes.length > 0 ? state.boxes[state.boxes.length - 1] : void 0;
  const previousSwimlane =
    state.previousSwimlaneNumber !== void 0
  ...
  swimlane.height =
    Math.max(diagramProps.swimlaneMinHeight, swimlane.maxHeight) +
    2 * diagramProps.swimlanePadding;
  const box = {
    x,
...
function hasSourceFrame(frame) {
  return (
    frame.sourceFrames !== void 0 &&
    frame.sourceFrames !== null &&
    frame.sourceFrames.length > 0
  );
}
...
function decidePositionRelation(state, _command) {
  const command = _command;
  if (
    isEmResetFrame(command.frame) ||
    isFirstFrame(command.index, command.frame)
  ) {
    return [];
  }
  ...
  if (command.sourceFrame) {
    ...
    sourceBox = findBoxByLineIndex(
      state.boxes,
      targetBox.swimlane.index,
      command.index - 1,
    );
  }
  if (sourceBox === void 0) {
...
var DEFAULT_EVENTMODELING_CONFIG2 =
  DEFAULT_CONFIG == null ? void 0 : DEFAULT_CONFIG.eventmodeling;
...
function renderD3Relation(
  ...
  return (relation) => {
    const sourceBoxY =
      relation.sourceBox.swimlane.y + diagramProps2.swimlanePadding;
    const targetBoxY =
      relation.targetBox.swimlane.y + diagramProps2.swimlanePadding;
    const upwards = dirUpwards(sourceBoxY, targetBoxY);
    const sourceX =
      relation.sourceBox.x + (relation.sourceBox.dimension.width * 2) / 3;
    const targetX =
      relation.targetBox.x + relation.targetBox.dimension.width / 3;
    let sourceY;
    let targetY;
    ...
    const relationStroke =
      themeVariables.emRelationStroke ?? relation.visual.stroke;
    diagram2
      .append('path')
...
function renderD3Swimlane(diagram2, maxR, diagramProps2, themeVariables) {
  return (swimlane) => {
    ...
    const oddBackground =
      themeVariables.emSwimlaneBackgroundOdd ?? 'rgb(250,250,250)';
    const backgroundStroke =
      themeVariables.emSwimlaneBackgroundStroke ?? 'rgb(240,240,240)';
    g.append('rect')
      .attr('x', 0)
      .attr('y', swimlane.y)
      .attr('rx', '3')
...
var draw = __name((txt, id, ver, diagObj) => {
  ...
  state.relations.forEach(
    renderD3Relation(diagram2, diagramProps2, arrowheadId, themeVariables),
  );
...
export { diagram };
//# sourceMappingURL=diagram-KO2AKTUF-WKO354WZ.js.map
