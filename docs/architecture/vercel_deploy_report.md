...
* **原因:** Vercelの管理画面に登録されていた DATABASE_URL の末尾に、余計なパス（例: /pancolle-db）や末尾のスラッシュ（/）が含まれていたため、Tursoの分散ルーター側が「該当するデータベースのアドレスが存在しない」と判断し、HTTP 404 (Not Found) を返していました。  
