...
export default function InputPage() {
  ...
  const [quantities, setQuantities] = useState<Record<string, number>>(
    [...ITEMS.bread, ...ITEMS.soup].reduce((acc, name) => ({ ...acc, [name]: 0 }), {})
  );

  const updateQuantity = (name: string, delta: number) => {
    setQuantities(prev => ({ ...prev, [name]: Math.max(0, prev[name] + delta) }));
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 pb-24">
      ...
      <main className="max-w-4xl mx-auto p-4 space-y-8">
        <section>
          <h2 className="text-zinc-400 text-xs font-bold mb-4 uppercase tracking-widest">パン</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {ITEMS.bread.map(name => (
              <ItemCard key={name} name={name} count={quantities[name]} onUpdate={updateQuantity} />
            ))}
          </div>
        </section>

        <section>
          <h2 className="text-zinc-400 text-xs font-bold mb-4 uppercase tracking-widest">スープ</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {ITEMS.soup.map(name => (
              <ItemCard key={name} name={name} count={quantities[name]} onUpdate={updateQuantity} />
            ))}
